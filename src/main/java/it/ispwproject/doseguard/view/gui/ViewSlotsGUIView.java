...
public class ViewSlotsGUIView extends PageGUIView {
    ...
    public void buildContent(BorderPane root,
        ...
        Runnable refreshList = () -> {
            ...
            if (current.isEmpty()) {
                ...
                for (TimeSlotBean s : current)
                    listBox.getChildren().add(buildSlotCard(s, isPast, onDelete));
            }
        };
        ...
        if (disponibili.isEmpty() && prenotati.isEmpty() && passati.isEmpty()) {
            Label empty = new Label("Non hai ancora creato slot.");
            empty.getStyleClass().add("register-label");
    ...
    private HBox buildSlotCard(TimeSlotBean s, boolean isPast, Consumer<TimeSlotBean> onDelete) {
        ...
        String dateStr = s.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
        Label dateTime = new Label(dateStr + "   Ore: " + s.getStartTime());
        ...
        if (isPast) {
            if (!s.isAvailable()) {
                ...
                status.setText("Visita Effettuata");
                status.getStyleClass().add("error-label");
                aggiungiDettagliPrenotazione(info, s);
            } else {
                dot.getStyleClass().add("past-label");
                status.setText("Non prenotato / Scaduto");
    ...
    private void aggiungiDettagliPrenotazione(VBox info, TimeSlotBean s) {
        if (s.getBookedByName() != null && !s.getBookedByName().isBlank()) {
            Label patient = new Label("Paziente: " + s.getBookedByName());
            patient.getStyleClass().add("register-label");
            info.getChildren().add(patient);
        }
    }
}