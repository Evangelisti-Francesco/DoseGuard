...
public class ViewSlotsView {
    ...
    public void mostraIntestazione() {
        CLIRenderer.intestazione("DoseGuard  –  I miei slot appuntamenti");
    }
        ...
        if (disponibili.isEmpty()) {
            ...
            for (TimeSlotBean s : disponibili) {
                System.out.printf("  %s  %s    %s Disponibile%n",
                        s.getDate(), s.getStartTime(), CLIRenderer.AVAIL);
            }
        }
        CLIRenderer.separatore();
    }

    public void mostraPrenotati(List<TimeSlotBean> prenotati, Map<Integer, String> patientBySlot) {
        ...
        if (prenotati.isEmpty()) {
            ...
            for (TimeSlotBean s : prenotati) {
                String nomePaziente = patientBySlot.getOrDefault(s.getId(), "Paziente");
                System.out.printf("  %s  %s    %s Prenotato  |  Paziente: %s%n",
                        s.getDate(), s.getStartTime(),
                        CLIRenderer.BOOKED, nomePaziente);
            }
        }
        CLIRenderer.separatore();
    }

    public void mostraPassati(List<TimeSlotBean> passati, Map<Integer, String> patientBySlot) {
        if (passati.isEmpty()) {
        ...
        for (TimeSlotBean s : passati) {
            ...
            String stato   = s.isAvailable() ? "Non utilizzato" : "Visita effettuata";
            String extra   = "";
            if (!s.isAvailable()) {
                String nomePaziente = patientBySlot.getOrDefault(s.getId(), "Paziente");
                extra = String.format("  |  Paziente: %s", nomePaziente);
            }
            System.out.printf("  %s  %s    %s %s%s%n",
                    s.getDate(), s.getStartTime(), simbolo, stato, extra);
        }
        CLIRenderer.separatore();
    }
